...
class Solarbank3(SolixBLEDevice):
    ...
    def _expansion_battery(self, slot: int) -> tuple[str | None, int | None, int | None]:
        ...
        Older firmware uses the marker
        ``63 01 <slot> <temperature> <format> <soc> <health>``.  The July
        2026 firmware keeps the record layout but changes the first marker
        byte to ``6a``.  The 16 ASCII bytes immediately before either marker
        are the battery serial.
